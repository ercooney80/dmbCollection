...
class RedBean_Driver_PDO implements RedBean_Driver {
  ...
  public function connect() {
    ...
    try {
      ...
      $this->pdo = new PDO(
              $this->dsn, $user, $pass, array(PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION,
...
class RedBean_OODBBean implements IteratorAggregate, ArrayAccess, Countable {
  ...
  public function beau($property) {
    ...
    if (
            strpos($property, 'own') === 0 || strpos($property, 'shared') === 0
    ) {
      return $property;
    }
    ...
    if (isset($this->$fieldLink) && $fieldLink !== $this->getMeta('sys.idfield')) {
      ...
      if (!$bean) {
        ...
        if ($this->withSql !== '') {
          ...
          $beans = $redbean->find(
                  $type, array('id' => array($this->properties[$fieldLink])), $this->withSql, $this->withParams);
  ...
  public function __set($property, $value) {
    ...
    if (isset($this->properties[$property . '_id']) && !( $value instanceof RedBean_OODBBean )
    ) {
    ...
    try {
      ...
      $fks = $this->adapter->getCell(
              "SELECT count(*)
				...
				CONSTRAINT_NAME <>'PRIMARY' AND REFERENCED_TABLE_NAME IS NOT NULL", array($db, $table)
      );
    ...
    try {
      ...
      $fks = $this->adapter->getCell(
              "SELECT
                ...
                AND PK.TABLE_NAME = ?", array($db, $table)
      );
